...
func (k KmsService) Verify(ctx context.Context, req awskms.VerifyInput) (*awskms.VerifyOutput, []error) {
	...
	if err = validator.RequiredPointer(req.Signature, "Signature"); err != nil {
		validationErrors = append(validationErrors, err)
	}
	...
	valid, err := key.Verify(req.Message, req.SigningAlgorithm, messageType, req.Signature)
	if err != nil {
		if errors.Is(err, cmk.ErrOperationNotSupported) {
			return nil, []error{
				kmserr.NewValidation(kmserr.CauseUnsupportedOperationException, "Verify not supported with KeySpec %s", metadata.KeySpec),
			}
		}
		return nil, []error{err}
	}
